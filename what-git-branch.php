...
class Plugin {
	...
	public function callback__dashboard_widget() : void {
		...
		foreach ( $this->repos as $repo ) {
			...
			$attr__class = '';
			$github_link = '';
			$head_ref    = sprintf( '<code>%s</code>', esc_html( $repo->get_head_ref() ) );
			...
			if ( $repo->is_root ) {
				...
				$head_ref    = sprintf( '<code class="what-git-branch">%s</code>', esc_html( $repo->get_head_ref() ) );
			}
			...
			printf(
				'<tr valign="top"%s><th scope="row" title="%s">%s</th><td>%s%s</td>',
				$attr__class,
				esc_attr( $repo->path ),
				esc_html( $name ),
				$head_ref,
	 ...
	 * @return array
	 */
	public function action__heartbeat_received( $response, array $data ) {error_log( __METHOD__ );
		...
		$response[ self::HEARTBEAT_KEY ] = array(
			'head_ref'   => $this->root_repo->get_head_ref(),
			'github_url' => $this->root_repo->get_github_url(),
		);
...
add_action( 'init', static function() : void {error_log( 'init' );
	if ( 'production' === wp_get_environment_type() ) {
		return;
	}
error_log( 'not production' );
	if ( ! current_user_can( 'manage_options' ) ) {
		return;
	}
error_log( 'can manage options' );
	if ( ! is_admin() && ! is_admin_bar_showing() ) {
		return;
	}
error_log( 'is admin or admin bar is showing' );
	Plugin::init();
} );